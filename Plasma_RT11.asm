# Plasma_RT11.asm (15-05-2017 para rodar no Plasma-v5)
#
...
inicio:	li	$t1,3		# Três tarefas
	...
	li	$t1,256		# Tempo entre ticks em clks (deve ser > tempo de back[34] + tempo de restauração[68]) 
	sw	$t1,0x90($gp)
...
task1:	lui	$gp,0x2000	#Ponteiro para os registradores
	...
	li	$t3,0x10002	# Tempo de sleep = 5 time slices, bit 16 = 1
    	li	$t4,0x00002	# Tempo de sleep = 5 time slices, bit 16 = 0
...
task2:	lui	$gp,0x2000	#Ponteiro para os registradores
	...
	li	$t2,0x00FF
    	li	$t3,0x10004	# Tempo de sleep = 5 time slices, bit 16 = 1
    	li	$t4,0x00004	# Tempo de sleep = 5 time slices, bit 16 = 0
task2_2:    	sw	$t0,0x0040($gp)	#Apaga todos os leds
    	nop
    	sw	$t1,0x0030($gp)	#Acende o led 1
    	nop
